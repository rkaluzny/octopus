...
use crate::attacks;
use crate::board::{Bitboard, Board, Color, PieceType};
...
pub fn evaluate(board: &Board) -> i32 {
    ...
    let all_pieces =
        board.color_bitboards[Color::White as usize] | board.color_bitboards[Color::Black as usize];
    ...
    let white_king_sq =
        (board.bitboards[PieceType::King as usize] & white_pieces).trailing_zeros() as u8;
    let black_king_sq =
        (board.bitboards[PieceType::King as usize] & black_pieces).trailing_zeros() as u8;
    ...
    let white_minor = (board.bitboards[PieceType::Knight as usize]
        | board.bitboards[PieceType::Bishop as usize])
        & white_pieces;
    let black_minor = (board.bitboards[PieceType::Knight as usize]
        | board.bitboards[PieceType::Bishop as usize])
        & black_pieces;
    ...
    let white_queen_on_start =
        (board.bitboards[PieceType::Queen as usize] & white_pieces & (1u64 << 3)) != 0;
    let black_queen_on_start =
        (board.bitboards[PieceType::Queen as usize] & black_pieces & (1u64 << 59)) != 0;
    let queen_penalty = if !white_queen_on_start
        && (board.bitboards[PieceType::Queen as usize] & white_pieces) != 0
    {
        -30
    } else {
        0
    };
    let queen_penalty_black = if !black_queen_on_start
        && (board.bitboards[PieceType::Queen as usize] & black_pieces) != 0
    {
        30
    } else {
        0
    ...
    let white_minors_starting = (board.bitboards[PieceType::Knight as usize]
        | board.bitboards[PieceType::Bishop as usize])
        & white_pieces
        & ((1u64 << 1) | (1u64 << 2) | (1u64 << 5) | (1u64 << 6));
    let black_minors_starting = (board.bitboards[PieceType::Knight as usize]
        | board.bitboards[PieceType::Bishop as usize])
        & black_pieces
        & ((1u64 << 57) | (1u64 << 58) | (1u64 << 61) | (1u64 << 62));
    let white_queen_extra = if !white_queen_on_start
        && (board.bitboards[PieceType::Queen as usize] & white_pieces) != 0
        && white_minors_starting != 0
    {
        -25
    } else {
        0
    };
    let black_queen_extra = if !black_queen_on_start
        && (board.bitboards[PieceType::Queen as usize] & black_pieces) != 0
        && black_minors_starting != 0
    {
        25
    } else {
        0
    ...
    let white_pawns_space =
        (board.bitboards[PieceType::Pawn as usize] & white_pieces & white_enemy_territory)
            .count_ones() as i32;
    let black_pawns_space =
        (board.bitboards[PieceType::Pawn as usize] & black_pieces & black_enemy_territory)
            .count_ones() as i32;
    let space_bonus =
        (white_space + white_pawns_space * 2 - black_space - black_pawns_space * 2) * 2;

    // Combine and scale by opening factor (fade in endgame)
    let dev_bonus = (white_dev - black_dev)
        + queen_penalty
        + white_queen_extra
        + queen_penalty_black
        + black_queen_extra
...
fn evaluate_pawns(board: &Board, color: Color) -> Score {
    ...
    // Doubled Pawns
    let enemy_pawns = board.bitboards[PieceType::Pawn as usize]
        & board.color_bitboards[color.opponent() as usize];
    for file in 0..8 {
...
fn evaluate_passed_pawns(board: &Board, color: Color) -> Score {
    ...
    let our_king =
        board.bitboards[PieceType::King as usize] & board.color_bitboards[color as usize];
    let king_sq = if our_king != 0 {
        our_king.trailing_zeros() as u8
    } else {
        0
    };
    let enemy_king = board.bitboards[PieceType::King as usize]
        & board.color_bitboards[color.opponent() as usize];
    let enemy_king_sq = if enemy_king != 0 {
        enemy_king.trailing_zeros() as u8
    } else {
        255
    };
    ...
    while bb != 0 {
        ...
        let adjacent =
            0u64 | if file > 0 {
                passed_bb & (0x0101010101010101u64 << (file - 1))
            } else {
                0
            } | if file < 7 {
                passed_bb & (0x0101010101010101u64 << (file + 1))
...
fn evaluate_king_safety_enhanced(
    board: &Board,
    color: Color,
    phase: i32,
    total_phase: i32,
) -> Score {
    let king_bb = board.bitboards[PieceType::King as usize] & board.color_bitboards[color as usize];
    if king_bb == 0 {
        return m(0, 0);
    }
    ...
    let minors = (board.bitboards[PieceType::Bishop as usize]
        | board.bitboards[PieceType::Knight as usize])
        & enemy_pieces
        & extended_zone;
    ...
    let enemy_rooks_queens = (board.bitboards[PieceType::Rook as usize]
        | board.bitboards[PieceType::Queen as usize])
        & enemy_pieces;
    let friendly_pawns =
        board.bitboards[PieceType::Pawn as usize] & board.color_bitboards[color as usize];
    ...
    if color == Color::White && king_idx < 56 {
        ...
        if (pawns >> front_sq) & 1 != 0 {
            shield_count += 1;
        }
        if king_idx % 8 > 0 && (pawns >> (front_sq - 1)) & 1 != 0 {
            shield_count += 1;
        }
        if king_idx % 8 < 7 && (pawns >> (front_sq + 1)) & 1 != 0 {
            shield_count += 1;
        }
...
fn evaluate_mobility(
    ...
    while bb != 0 {
        let sq = bb.trailing_zeros() as u8;
        let moves =
            (attacks::get_knight_attacks(sq) & !our_pieces & !pawn_attacks).count_ones() as i32;
    ...
    while bb != 0 {
        let sq = bb.trailing_zeros() as u8;
        let moves = (attacks::get_bishop_attacks(sq, all_pieces) & !our_pieces & !pawn_attacks)
            .count_ones() as i32;
    ...
    while bb != 0 {
        let sq = bb.trailing_zeros() as u8;
        let moves = (attacks::get_rook_attacks(sq, all_pieces) & !our_pieces & !pawn_attacks)
            .count_ones() as i32;
    ...
    while bb != 0 {
        let sq = bb.trailing_zeros() as u8;
        let moves = ((attacks::get_rook_attacks(sq, all_pieces)
...
fn evaluate_outposts_and_minors(board: &Board, color: Color, _all_pieces: Bitboard) -> Score {
    ...
    let _enemy_back_rank = if color == Color::White {
    ...
    while bb != 0 {
        ...
        if (sq_bb & pawn_safe_squares & in_enemy_half) != 0 {
            ...
            if is_protected
                || (sq_bb
                    & attacks::get_king_attacks(
                        board.bitboards[PieceType::King as usize].trailing_zeros() as u8,
                    ))
                    != 0
            {
    ...
    while bb != 0 {
        ...
        let has_friendly =
            (board.bitboards[PieceType::Pawn as usize] & our_pieces & file_mask) != 0;
        let has_enemy = (board.bitboards[PieceType::Pawn as usize]
            & board.color_bitboards[color.opponent() as usize]
            & file_mask)
            != 0;
